...
calc_max <- unlist(lapply(1:8,function(x){max(stati[[x]][,101])}))
tab_global <- sapply(1:8,function(x){apply(stati[[x]],2,max)})
...
## value 101 is the real value
## caution! it takes a while
#stati is a list of 9 variables, each w/ matrix(0,46,101)
